...
y <- file.path(inputDir,data.file.name)
...
seed=12345
write_itr=200
dir_out = file.path(table.path,"sfamix")
dir.create(dir_out)
lam_method="matrix"
x_method="dense"
tol=1e-5
bicmix.path=file.path("./C","BicMix")

y="~/BicMixC/AOAS/data/n.effects30_std.err1_seed1_std.effect2_b0.5_denseTRUE.txt"

BicMixR(y=y,nf=100,a=0.5,b=0.5,c=0.5,d=0.5,e=0.5,f=0.5,itr=5001,seed=12345,write_itr=500,dir_out=dir_out,lam_method="matrix", x_method="dense", tol=0.0001, bicmix.path=bicmix.path)


BicMixR <- function(y=NULL,nf=100,a=0.5,b=0.5,c=0.5,d=0.5,e=0.5,f=0.5,itr=5001,interval=200, sep="space", seed=NULL,write_itr=500,dir_out=NULL, lam_method=NULL, x_method=NULL, tol=NULL, bicmix.path=NULL){
   
   tmpf <- tempfile(pattern = "", tmpdir = "")
   tmpf <- basename(tmpf)
   dir_out <- file.path(dir_out,paste0(basename(y),"_",tmpf))
   dir.create(dir_out)
   

   cmd <- paste0(
       bicmix.path, " --nf ", nf, " --y ", y, " --dir_out ", dir_out, " --sep ", sep, " --a ", a, " --b ", b, " --c ", c, " --d ", d, " --e ", e, " --f ", f, " --seed ", seed, " --itr ", itr, " --interval ", interval, " --write_itr ", write_itr, " --tol ", tol,
       " --lam_method ", lam_method, " --x_method ", x_method     
   )

    if(file.exists(file.path(dir_out,"LAM"))){
        lam <- read.table(file.path(dir_out,"LAM"))
        ex <- read.table(file.path(dir_out,"EX"))
        z <- read.table(file.path(dir_out,"Z"))
        exx <- read.table(file.path(dir_out,"EXX"))
        psi <- read.table(file.path(dir_out,"PSI"))
       seed <- read.table(file.path(dir_out,"seed"))
    }else{
        lam <- read.table(file.path(dir_out,paste0("LAM_",itr)))
        ex <- read.table(file.path(dir_out,paste0("EX_",itr)))
        z <- read.table(file.path(dir_out,paste0("Z_",itr)))
        exx <- read.table(file.path(dir_out,paste0("EXX_",itr)))
        psi <- read.table(file.path(dir_out,paste0("PSI_",itr)))
       seed <- read.table(file.path(dir_out,"seed"))
    }
    return(list(lam=lam,ex=ex,z=z,exx=exx,psi=psi,seed=seed))
}
...
run_sim <- function(param.config, itr=2001, inputDir=NULL, outputDir = NULL, script.path = NULL, bfrm.path=NULL, matlab.path = NULL, nfs=20, nf=30, ng= 1000, ns=500, min_fac=20,step=2, mc.cores=5){
    results<- mclapply(((1:nrow(param.config))),function(i){
        ...
        if(method == "SFAmix"){
            ...
            results <- BicMixR(y=data$y, nf = 50, out_dir=outputDir.sfamix, rsd = 123, tol=1e-10, itr = 1001)
            lam <- results$lam
            z <- results$z
